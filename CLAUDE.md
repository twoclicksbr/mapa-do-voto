...
<!-- Atualizado em: 03/04/2026 10:15 -->
...
- `php artisan tse:import-zone-geometry {--uf=} {--ano=}` — popula campo `geometry` das zonas em `maps.zones` a partir do GeoJSON dos municípios (tbrugz geodata-br)
- `php artisan tse:geocode-voting-locations {--uf=} {--limit=} {--concurrency=10}` — geocodifica `maps.voting_locations` via Google Maps Geocoding API usando `Http::pool()` em paralelo; requer `GOOGLE_MAPS_KEY` no `.env`; pula registros que já têm lat/lng
...
| Arquivo | Descrição |
|---------|-----------|
| `src/pages/home/page.tsx` | Página principal. Sistema de abas: Gabinetes (só `master`), Mapa, Atendimentos, Agenda, Alianças, Finanças, Configurações. Detecta `isMaster` pelo subdomínio. Aba Gabinetes carrega `GET /api/tenants` e exibe `GabinetesDataGrid`. Aba Configurações exibe `AppMegaMenu` com seções: Permissões (DataGrid + Modal com DnD), Pessoas (DataGrid + Modal), Tipos de Pessoa, Tipos de Contato, Tipos de Endereço, Tipos de Documento. Botão split só visível no `isMaster`. Navbar exibe dropdown de seleção de gabinete. Estado `finTitleInitialTab` controla qual aba do `FinTitleModal` abre: `onEdit` → `"geral"`, `onBaixar` → `"baixar"`; resetado para `"geral"` ao fechar. `FinCompositionModal.onConfirm` chama `POST /fin-titles/compose` e recarrega o DataGrid correspondente. `FinTitlesFilterModal` abre ao clicar em Pesquisar nos DataGrids de títulos; filtragem client-side via `applyFinFilters(list, filters)` + `useMemo` — **estados separados por tipo**: `finTitlesFilters` (expense) e `finTitlesIncomeFilters` (income). `finTitlesClearKey`/`finTitlesIncomeClearKey` — incrementados após reload ou bulk cancel para resetar seleção. Seção `fin-extract`: `FinExtractFilterModal` (Pesquisar) + `FinExtractModal` (Novo Lançamento); filtragem client-side via `applyExtractFilters(list, filters)` + `useMemo`. **Seção `fin-dashboard`**: renderiza `<FinDashboardTab dateFrom dateTo />` — filtro de período via `finDashDateValue` (DateSelectorValue, default = ano atual); `finDashDateCustom` controla visibilidade do botão X (só aparece quando alterado do padrão); `DATE_I18N` config PT-BR com nomes de meses completos em `monthsShort`, trimestres "1° Trimestre…" e semestres "1° Semestre/2° Semestre"; `extractDateRange(v)` em `page.tsx` converte `DateSelectorValue` → `{dateFrom, dateTo}` com suporte a `rangeStart`/`rangeEnd` para intervalos de mês/trimestre/semestre/ano (`endOfMonth`/`endOfQuarter`/`endOfYear` corretos). **Seção `fin-wallet`**: renderiza `<FinWalletTab />`. **Seção `settings-dashboard`** (Configurações): renderiza `<SettingsDashboardTab />`. **Modais de pesquisa avançada** em todas as seções: Gabinetes (`GabinetesFilterModal`), Tipos de Pessoa, Tipos de Contato, Tipos de Endereço, Tipos de Documento, Pessoas (`PeopleFilterModal` com `applyPeopleFilters()` + `matchesDateFilter()`), Permissões, Bancos (`FinBanksFilterModal`), Tipos de Modalidade (`FinPaymentMethodTypesFilterModal`), **Modalidades** (`FinPaymentMethodsFilterModal` com `applyFinPaymentMethodsFilters()` + `filteredFinPaymentMethods`), **Departamentos** (`DepartmentsFilterModal` com `applyDepartmentsFilters()` + `filteredDepartments`), **Plano de Contas** (`FinAccountsFilterModal` com `filterFinAccountTree()` + `flattenFinAccounts()` + `filteredFinAccounts`), Tipos de Evento (`EventTypesFilterModal`) — cada seção tem estado `*FilterOpen` + `*Filters`. Todos os footers inline foram substituídos pelo componente `PageFooter`. Aba `metrics` (Agenda) renderiza `<AgendaTab />`. Aba `activity` (Atendimentos) renderiza `<AttendanceTab />`. Layout: `html/body/#root` com `overflow: hidden`; containers internos usam `flex-1 min-h-0`. Interface `Tenant` inclui `tenant_id?`, `plan_id?`, `logo_original?`, `logo_md?`, `logo_sm?`. Importa `LandPlot` do lucide-react; mapa de ícones de seção usa `'Planos': LandPlot` e título da seção plans renderiza `<LandPlot className="size-5" />`. **`document.title` dinâmico**: `useEffect` atualiza o título da aba do browser com base em `activeTab`, `finSection` e `settingsSection` — formato `"{Seção} | Mapa do Voto"` (ex: "Bancos | Mapa do Voto", "Planos | Mapa do Voto"). Aba Configurações passa `isMaster={isMaster}` para `<AppMegaMenu>`, que oculta a seção "Planos" para não-masters. **Aba `map-novo` (Mapa novo)**: sidebar própria com busca de candidato (busca `GET /candidates` ao abrir, com debounce na pesquisa), seleção de candidato (estado `mapNovoCandidate`), stats via `GET /candidacies/{id}/stats` (recalcula ao mudar city/zone/VL). **Skeletons de loading** (`animate-pulse`): lista de candidatos exibe skeleton com avatar circular + linhas de texto enquanto `mapNovoLoading`; stats exibe skeleton com tabs de turno + card de stats enquanto `mapNovoStatsLoading` (preserva `mapNovoLoadProgress` na barra interna do skeleton); estado sem candidato selecionado exibe skeleton com switch row + turno tabs + stats card (substituiu placeholder grid de botões Metronic). Drill-down de filtros: **Cargos estaduais** — switch "Exibir Cidades" → seletor de cidade (busca `GET /candidacies/{id}/cities`) → switch "Exibir Zonas Eleitorais" + switch "Colégio Eleitoral"; **Cargos municipais** — switches de Zonas e Colégio diretamente (usa `mapNovoCandidate.city_id`). `useEffect` de zonas e VL detectam `isMunicipal` e derivam `cityId` da candidatura quando não há cidade selecionada. Cascata de limpeza: trocar candidato limpa tudo; desmarcar zonas limpa zona+colégio; selecionar nova zona limpa colégio; marcar zonas (ON) limpa apenas colégio selecionado (preserva lista de colégios). **BrazilMap integrado**: `brazilMapRef` + `<BrazilMap ref={brazilMapRef} candidate={mapNovoCandidate} showCities={...} selectedCityName={...} showZones={...} zones={...} selectedZoneId={...} votingLocations={...} pessoasMarkers={...} atendimentosMarkers={...} onCityClick={...} onZoneClick={...} onVotingLocationClick={...} />`; ao selecionar cidade na sidebar → `brazilMapRef.current?.focusCity(c.name)` destaca no mapa; ao clicar num município → `onCityClick` seta `mapNovoPendingCity` e ativa "Exibir Cidades"; `useEffect` resolve `mapNovoPendingCity` quando `mapNovoCities` carrega; botão X da cidade chama `brazilMapRef.current?.fitState()`; ao entrar na aba `map-novo` → se sem candidato `fitBrazil()`, senão `invalidateSize()` (sem delay de refit para preservar zoom). **`forceMount`** na TabsContent `map-novo` + `style={{ display: activeTab === 'map-novo' ? undefined : 'none' }}` — mantém componente montado para preservar estado do mapa ao trocar abas. **Botão crosshair context-aware**: sem candidato → `fitBrazil()`; com zona selecionada → `focusZone()`; com cidade → `focusCity(name)`; senão → `fitState()`. **Checkboxes de overlay**: apenas `pessoas` (azul) e `atendimentos` (amarelo) — substituem os 5 anteriores (`overview/activity/metrics/reports/alerts`); `mapNovoPessoasMarkers` / `mapNovoAtendimentosMarkers` carregados via `GET /api/addresses/map?modulo=people|attendances` ao marcar. **Estado `mapNovoZones`** agora inclui campo `geometry` (GeoJSON) para renderizar polígonos no mapa. **Estado `mapNovoVotingLocations`** agora inclui `latitude`/`longitude`. |
| `src/components/map/mapa-do-voto-map.tsx` | Mapa Leaflet + CandidateCard + StatsCard (overlay flutuante) + CitySearch interno + heatmap + botões flutuantes. `CitySearch` interno: prop `stateUf`, busca normalizada (unaccent via `normalizeName`), exibe "CIDADE - UF". `StatsCard`: exibe "0 votos nesta cidade" quando city sem votos. `MapCore`: `focusedCityBoundsRef` guarda bounds da cidade focada (clique ou CitySearch); crosshair usa cidade quando disponível senão estado; `clearCityHighlight` e toggle "Exibir Cidades" voltam zoom ao estado via `polygonLayerRef` com maxZoom:7. Termômetro do heatmap: tooltip hover com valor abreviado (250/2.5k/1.5m votos) seguindo mouse verticalmente. `fmtShort()` formata números. Prop `showOverlays?: boolean` (default `true`) — quando `false` oculta CandidateCard, StatsCard e botões flutuantes (usado no split direito ou mapa compacto). Painel de visualização inclui switch **Atendimentos** (`showAttendances` do contexto, ícone `MonitorCloud`). |
| `src/components/map/brazil-map.tsx` | Mapa Leaflet standalone usado na aba **Mapa novo**. Props: `candidate`, `showCities`, `selectedCityName`, `onCityClick`, `showZones`, `zones` (inclui `geometry` GeoJSON), `selectedZoneId`, `onZoneClick`, `votingLocations` (inclui `latitude`/`longitude`), `selectedVotingLocationId`, `onVotingLocationClick`, `pessoasMarkers`, `atendimentosMarkers`. Handle ref (`BrazilMapHandle`): `zoomIn`, `zoomOut`, `fitBrazil`, `fitState`, `focusCity(name)`, **`focusZone()`** (re-foca zona selecionada), **`invalidateSize()`** (invalida tamanho ao trocar aba sem recriar mapa). `MapCore` props expandidos para receber todo o estado de seleção. Layers: `selectedCityLayerRef` (destaca só a cidade selecionada), `zonesLayerRef` (polígonos de zonas — esconde cidade selecionada quando ativo), `vlMarkersRef` (colégios eleitorais — círculo verde/vermelho 30px com ícone house), `pessoasMarkersRef` (círculo azul #3B82F6 28px com ícone people), `atendimentosMarkersRef` (círculo amarelo #F59E0B 28px com ícone monitor). `focusZoneRef` expõe função para refoco da zona selecionada. `showCitiesRef` (useRef síncrono) evita race condition ao carregar GeoJSON. Estado do polígono do estado: agora tem `fillOpacity:0.04` + tooltip da UF; carrega em segundo plano quando `showCities=true`. Exporta interfaces `Zone`, `VotingLocation`, `AddressMarker`. `UF_TO_IBGE` + `normalizeName` para match de cidades. |
...
| `src/components/mapa-do-voto/sidebar.tsx` | Painel lateral: stats reais, turno dinâmico, badge Status TSE flutuante. Props: `inline?: boolean` (quando `true` renderiza sem `SidebarHeader`/`SidebarFooter`, para uso embutido na aba Mapa novo), `onClose?: () => void`. Exporta `MapaDoVotoSidebarContent`. Inclui `ActiveCandidateCard` (card do candidato com avatar, badge de partido com gradiente, botão X). |
| `src/components/map/active-candidate-context.tsx` | Contexto global: activeCandidate, setActiveCandidate, showCities, setShowCities, showCard, setShowCard, **showAttendances, setShowAttendances**, mapClickedCity (inclui city_id), focusCityOnMap, clearCityHighlight |
...
| `src/components/gabinetes/gabinetes-data-grid.tsx` | Data grid de tenants: colunas ID, Nome (clicável → `onEdit`, com avatar circular — logo_sm ou ícone Building2 fallback), Subdomínio (link externo), Validade (badge com alerta vencimento), Status, Ações (editar/excluir); prop `onEdit`. Interface `Tenant` inclui `plan_id?`, `logo_sm?`, `logo_md?` |
...
| `src/components/reui/timeline.tsx` | Timeline horizontal customizada (sem shadcn CLI) — componentes: `Timeline`, `TimelineItem`, `TimelineHeader`, `TimelineSeparator`, `TimelineIndicator`, `TimelineTitle`, `TimelineContent`. Usa React context para `value` (step atual) e `orientation`. `TimelineItem` adiciona atributo `data-completed` quando `status === "completed"`. |
| `src/components/ui/avatar.tsx` | Exporta `Avatar`, `AvatarFallback`, `AvatarImage`, `AvatarIndicator`, `AvatarStatus`, `AvatarGroup` (novo — wrapper `flex -space-x-2.5` para grupos sobrepostos de avatares), `avatarStatusVariants`. |
| `src/components/gabinetes/gabinete-edit-modal.tsx` | Modal de edição de tenant — painel esquerdo (logo com upload/remoção/lightbox via POST/DELETE `/tenants/{id}/avatar`, dados, edição inline) + painel direito com abas: **Pessoas** (grid lazy com avatar/nome/tipo/status + botão Editar que abre `PeopleModal` + botão "Adicionar pessoa" que abre `PeopleModal` inline; `handleSaved` faz upsert — adiciona pessoa nova à lista; busca `GET /tenants/{id}/people`), **Contatos**, **Endereços** (mapa Leaflet + ViaCEP), **Documentos**, **Notas**, **Arquivos** (`PeopleFilesTab`), **Plano** (grid de cards de planos; card selecionado com `border-primary bg-primary/5` + ícone `Check`; `handlePlanSelect` faz `PUT /tenants/{id}` com `plan_id`, rollback em erro + alert). Sem aba "Pessoa" singular — substituída pela aba "Pessoas" (plural) em primeiro lugar. Validade usa `BirthDatePicker`. Interface `Tenant` inclui `plan_id?`, `logo_original?`, `logo_md?`, `logo_sm?`. Estado: `plans`, `selectedPlanId`, `planSaving`, `logoOriginal`, `logoMd`, `avatarLoading`, `avatarLightbox`. |
...
| `src/components/common/app-mega-menu.tsx` | Wrapper reutilizável do MegaMenu do Layout 1 — props: `onNavigate`, `activeSection` (destaca o botão do módulo ativo), `isMaster` (quando `false`, filtra a seção `plans` do menu — não-masters não veem a aba Planos). Item "Planos" usa ícone `LandPlot`. Importa `MENU_MEGA` e passa menu filtrado ao `MegaMenu`. |
...
| `src/components/people/people-modal.tsx` | Modal de pessoas: Create (small) + Detail (large 2 painéis). Abas: Geral, Contatos, Endereços, Documentos, Notas, Arquivos, Usuário, Permissões. Tabs de sub-recursos (Contatos, Endereços, Documentos, Notas) importadas de `entity-tabs.tsx` via prop `basePath={`/people/${id}`}` — removidas implementações inline, `applyMask`, `PhoneInput`/`parsePhoneNumber`/`formatPhoneNumber` e ícones `Minus`/`LocateFixed`. Aba Permissões: grid 3 colunas de Frame cards com Checkbox por ação. `PeopleModalProps` e `CreatePersonModal` aceitam `tenantId?: number`; ao criar pessoa com `tenantId` inclui `tenant_id` no payload do POST `/people`. **Sem lógica de evento de aniversário** — aniversários são gerados dinamicamente pela `agenda-tab` a partir de `birth_date`. |
...
| `src/components/reui/kanban.tsx` | **Novo.** Primitivo Kanban genérico com `@dnd-kit`. Props: `value` (`Record<string, T[]>`), `onValueChange`, `getItemValue`. Lida com DnD cross-coluna (via `handleDragOver`) e reordenação na mesma coluna (via `handleDragEnd` + `arrayMove`). `KanbanColumnContent` é droppable mesmo quando vazia. Exporta: `Kanban`, `KanbanBoard`, `KanbanColumn`, `KanbanColumnContent`, `KanbanItem`, `KanbanItemHandle`, `KanbanOverlay`. |
| `src/components/common/entity-tabs.tsx` | **Novo.** Tabs de sub-recursos reutilizáveis com prop `basePath: string` (ex: `/attendances/5`, `/people/3`). Exporta: `ContactsTab` (PhoneInput para WhatsApp, `applyMask` para tipos com máscara, salva dígitos puros), `NotesTab`, `DocumentsTab`, `AddressesTab` (mapa Leaflet + BrasilAPI v2 + geocode Google Maps), `applyMask`, e interfaces `ContactItem`, `AddressItem`, `DocumentItem`, `NoteItem`. Elimina duplicação entre `people-modal.tsx` e `attendance-modal.tsx`. |
...
| `src/components/attendances/attendance-tab.tsx` | Aba Atendimentos — carrega `GET /attendances`, filtragem client-side via `applyAttendanceFilters()`. Header com breadcrumb, badge de contagem, botões Pesquisar e Novo Registro. **Toggle Grid/Kanban** — `viewMode` state (`"grid" \| "kanban"`, default `"grid"`); ícones `Table2` (grid) e `LayoutGrid` (kanban); renderiza `AttendanceKanban` ou `AttendanceDataGrid` conforme modo. `handleSaved`: ao criar, reabre modal para edição imediata. `handleReorder`: DnD otimístico com `PUT /attendances/reorder`. `onStatusChange` atualiza status no estado local; `onReorderAll` substitui lista inteira. |
| `src/components/attendances/attendance-data-grid.tsx` | DataGrid de atendimentos com DnD. Interface `Attendance` inclui `people_id`, `people_name`, `people_photo_sm`, `title`, `status`, `priority`, `order`, `opened_at`, `resolved_at`, `people[]` (AttendancePerson). Colunas: drag handle, checkbox, Pessoa (avatar + nome), Título, Status (badge), Prioridade (badge), Abertura, Ações (editar/excluir). Interface `AttendancePerson` inclui `people_id`, `name`, `photo_sm`, `active`. |
| `src/components/attendances/attendance-modal.tsx` | Modal criar/editar atendimento com abas (Geral, Pessoas, Histórico, Contatos, Documentos, Endereços, Notas). Tabs de sub-recursos importadas de `entity-tabs.tsx` via prop `basePath={`/attendances/${id}`}`. `CreateAttendanceModal`: botão `+` ao lado do label "Pessoa" abre `PeopleModal` inline para criar nova pessoa; busca `GET /type-people` ao abrir; pessoa criada adicionada à lista local e selecionada automaticamente. Selects de status/prioridade com `indicatorPosition="right"`. Dot de prioridade `size-3`. Aba Endereços exibe badge de contagem quando `addresses.length > 0`. |
| `src/components/attendances/attendance-kanban.tsx` | **Novo.** Board kanban de atendimentos com 3 colunas: `aberto` (vermelho), `em_andamento` (amarelo), `resolvido` (verde). Usa `Kanban`/`KanbanBoard`/`KanbanColumn`/`KanbanColumnContent`/`KanbanItem`/`KanbanItemHandle`/`KanbanOverlay` de `reui/kanban.tsx`. `AttendanceCard`: título clicável (blue-600 hover underline), badge de prioridade, data de abertura, avatares de pessoas (até 4 + overflow badge). Ao mover card entre colunas: `PUT /attendances/{id}` com novo status + `onStatusChange`. Ao reordenar na mesma coluna: `PUT /attendances/reorder` com array global de `{id, order}` + `onReorderAll`. Props: `attendances`, `isLoading`, `onEdit`, `onDelete`, `onStatusChange`, `onReorderAll`. |
| `src/components/attendances/attendance-filter-modal.tsx` | Modal de pesquisa avançada de atendimentos — filtros: Título, Pessoa, Status (multi), Prioridade (multi), Período (DateSelector). Exporta `AttendanceFilterModal`, `AttendanceFilters`. |
| `src/components/agenda/agenda-tab.tsx` | Aba Agenda com FullCalendar (dayGridMonth / timeGridWeek / timeGridDay / listWeek). Sidebar esquerda: data de hoje + eventos do dia (filtro `modulo !== "people"`) + legenda de tipos. `ViewSwitcher` dropdown (Mês/Semana/Dia/Lista). `handleNewEvent` abre `EventModal` com próxima hora cheia. `dateClick` abre `EventModal` com data/hora do clique. **Títulos financeiros pendentes integrados ao calendário**: busca `GET /fin-titles?status=pending&date_from&date_to` em paralelo com `GET /events`; renderiza como eventos `allDay` com cores de `Financeiro Pagar`/`Financeiro Receber`; clicar abre `FinTitleDetailModal`. Clicar num evento agenda abre `EventDetailModal` com botão Editar (para eventos sem `modulo`). `allDaySlot=true`. `todayIso()` usa data local (não UTC). `_fc_id` usado para eventos recorrentes. **Aniversários gerados dinamicamente**: interface `CalendarPerson` (`{id, name, birth_date}`); estado `calendarPeople` + fetch `GET /people` no useEffect inicial; `loadEvents` filtra eventos do DB com `modulo === "people"` (registros antigos), gera eventos de aniversário a partir de `calendarPeople[].birth_date` para o range visível (id `birthday-{personId}-{year}`); deps de `loadEvents` incluem `eventTypes` e `calendarPeople`. |
...
| `src/components/financeiro/fin-dashboard-tab.tsx` | **Dashboard Financeiro** — Props: `{ dateFrom?: string; dateTo?: string }` (período externo). KPIs (Saldo em Bancos, A Pagar Pendente, A Receber Pendente, Saldo Líquido). **Fluxo de Caixa**: `AreaChart` (não mais BarChart) com 12 meses (-10 atual +1), gradiente verde/vermelho, tooltip customizado exibindo "Mês Ano" + valores formatados em BRL. **Status dos Títulos**: dois gauges meia-lua (`PieChart` com `startAngle={180} endAngle={0}`) lado a lado — Pagar (vermelho) e Receber (verde); `typeCounts` useMemo agrega counts + amounts por tipo; `isAnimationActive={false}` nos Pie para evitar tooltip abrindo no mount; tooltip customizado por gauge. Listas **Títulos Vencidos** e **A Vencer (30 dias)**: badges Pagar/Receber + cor verde para receitas. Todos os dados filtrados pelo `filterFrom`/`filterTo` recebidos via props. Busca em paralelo: `GET /fin-banks`, `GET /fin-titles?type=expense&status=pending&date_from&date_to`, `GET /fin-titles?type=income&status=pending&date_from&date_to`, `GET /fin-titles?date_from&date_to`, `GET /fin-extract?date_from&date_to`. |
...
| POST | `/api/tenants/{id}/avatar` | Bearer | — | Upload de logo do gabinete — gera 3 versões: original.jpg, md.jpg, sm.jpg via Intervention Image |
| DELETE | `/api/tenants/{id}/avatar` | Bearer | — | Remove logo do gabinete e arquivos do storage |
...
| GET | `/api/candidacies/{id}/stats?city_id=&zone_id=&voting_location_id=` | Bearer | — | Retorna votos por turno: qty_votes, %, brancos, nulos, legenda, total partido, status TSE — query única CTE com partition pruning por year; escopo: `voting_location_id` > `zone_id` > `city_id` > padrão cargo; resultado cacheado com `Cache::forever` por chave `candidacy_stats_{id}_city_{cid}_zone_{zid}_vl_{vlid}`; **fallback**: se candidato não teve votos no escopo mas o escopo tem dados (outros candidatos), retorna `qty_votes=0` com totais reais do escopo em vez de `[]` vazio |
...
| GET | `/api/cities/{id}/zones?candidacy_id=` | Bearer | — | Lista zonas eleitorais de uma cidade; com `candidacy_id` retorna `qty_votes` agregado por zona (maior turno, filtrando por year para partition pruning) + campo **`geometry`** (GeoJSON); sem candidacy retorna id + zone_number + geometry |
| GET | `/api/cities/{id}/voting-locations?candidacy_id=&zone_id=` | Bearer | — | Lista colégios eleitorais de uma cidade; com `candidacy_id` retorna `qty_votes` agregado; agora inclui campos **`latitude`** e **`longitude`** em ambos os modos; `zone_id` opcional filtra por zona; ordenado por qty_votes DESC, nome ASC |
| GET | `/api/addresses/map?modulo=` | Bearer | — | Retorna marcadores de endereço para o mapa; `modulo=people` → join com `gabinete_master.people`; `modulo=attendances` → join com `gabinete_master.attendances`; filtra `lat/lng NOT NULL`, `active=true`, `deleted_at IS NULL`; retorna id, record_id, lat, lng, logradouro, numero, bairro, cidade, name |
...
| GET | `/api/attendances` | Bearer | — | Lista atendimentos com filtros: `status`, `priority`, `people_id`, `date_from`, `date_to`; ordenados por `order` + id desc |
| PUT | `/api/attendances/reorder` | Bearer | — | Reordena atendimentos em lote: `{items: [{id, order}]}` |
| GET | `/api/attendances/{id}` | Bearer | — | Detalhe do atendimento com histórico |
| POST | `/api/attendances` | Bearer | — | Cria atendimento; auto-cria entrada no histórico com status `aberto` |
| PUT | `/api/attendances/{id}` | Bearer | — | Atualiza atendimento; gera entrada no histórico se `status` mudar; auto-preenche `resolved_at` |
| DELETE | `/api/attendances/{id}` | Bearer | — | Soft delete do atendimento |
| GET | `/api/attendances/{id}/people` | Bearer | — | Lista pessoas vinculadas ao atendimento |
| POST | `/api/attendances/{id}/people` | Bearer | — | Vincula pessoa ao atendimento (upsert com restore se deletado) |
| DELETE | `/api/attendances/{id}/people/{pid}` | Bearer | — | Remove vínculo de pessoa |
| GET | `/api/attendances/{id}/history` | Bearer | — | Lista histórico de status ordenado por data |
| POST | `/api/attendances/{id}/history` | Bearer | — | Adiciona entrada manual no histórico |
| GET | `/api/attendances/{id}/contacts` | Bearer | — | Lista contatos do atendimento (modulo=attendance) |
| POST | `/api/attendances/{id}/contacts` | Bearer | — | Adiciona contato |
| PUT | `/api/attendances/{id}/contacts/{cid}` | Bearer | — | Atualiza contato |
| DELETE | `/api/attendances/{id}/contacts/{cid}` | Bearer | — | Remove contato |
| GET | `/api/attendances/{id}/documents` | Bearer | — | Lista documentos do atendimento (modulo=attendance) |
| POST | `/api/attendances/{id}/documents` | Bearer | — | Adiciona documento |
| PUT | `/api/attendances/{id}/documents/{did}` | Bearer | — | Atualiza documento |
| DELETE | `/api/attendances/{id}/documents/{did}` | Bearer | — | Remove documento |
| GET | `/api/attendances/{id}/addresses` | Bearer | — | Lista endereços do atendimento (modulo=attendances) |
| POST | `/api/attendances/{id}/addresses` | Bearer | — | Adiciona endereço |
| PUT | `/api/attendances/{id}/addresses/{aid}` | Bearer | — | Atualiza endereço |
| DELETE | `/api/attendances/{id}/addresses/{aid}` | Bearer | — | Remove endereço |
| GET | `/api/attendances/{id}/notes` | Bearer | — | Lista notas do atendimento (modulo=attendances) |
| POST | `/api/attendances/{id}/notes` | Bearer | — | Adiciona nota |
| DELETE | `/api/attendances/{id}/notes/{nid}` | Bearer | — | Remove nota |
...
| Tabela | Descrição |
|--------|-----------|
| `gabinete_master.tenants` | Gabinetes (id, name, slug unique, schema unique, active, valid_until, plan_id nullable FK → plans, logo_path nullable) — slug identifica o tenant pelo subdomínio |
...
| `gabinete_master.attendances` | Atendimentos (people_id nullable, title, description, address, lat, lng, status, priority, order, opened_at, resolved_at) |
| `gabinete_master.attendance_people` | Pivot atendimento ↔ pessoa (attendance_id, people_id, active) — UNIQUE(attendance_id, people_id) |
| `gabinete_master.attendance_history` | Histórico de status dos atendimentos (attendance_id, status, notes) |
...
| `maps.voting_locations` | Locais de votação (id, zone_id, tse_number, name, address, **latitude**, **longitude**) — lat/lng populados via `tse:geocode-voting-locations` com Google Maps API |
...
- `Tenant` (`gabinete_master.tenants`) — com SoftDeletes; `$fillable`: `name`, `slug`, `schema`, `active`, `valid_until`, `plan_id`, `logo_path`; cast `valid_until` → `date`
...
- `Attendance` (`gabinete_master.attendances`) — com SoftDeletes; `$fillable`: `people_id`, `title`, `description`, `address`, `lat`, `lng`, `status`, `priority`, `opened_at`, `resolved_at`, `order`; casts: `lat`/`lng` → `float`, `opened_at`/`resolved_at` → `datetime`; `belongsTo(People)`, `hasMany(AttendanceHistory)`, `hasMany(AttendancePeople)`
- `AttendancePeople` (`gabinete_master.attendance_people`) — com SoftDeletes; `$fillable`: `attendance_id`, `people_id`, `active`; cast `active` → `boolean`; `belongsTo(People, 'people_id')`
- `AttendanceHistory` (`gabinete_master.attendance_history`) — com SoftDeletes; `$fillable`: `attendance_id`, `status`, `notes`; `belongsTo(Attendance)`
...
| `000051`–`000052` | `gabinete_master` | attendances (com `priority` varchar default `media`, `order` int default `0`, `people_id` nullable), attendance_history |
| `000054` | `gabinete_master` | attendance_people (attendance_id FK cascade, people_id bigint sem FK, active bool, UNIQUE(attendance_id, people_id)) |
...
| `000077` | `gabinete_master` | Adiciona `logo_path` (string nullable) e `plan_id` (unsignedBigInteger nullable) em `tenants`; seed do tenant "Mapa do Voto" define `plan_id` dinamicamente buscando o id do plano "Mapa + Gabinete - Enterprise" |
...
| `api/app/Http/Controllers/TenantController.php` | `index`, `store`, `update`, `person`, `storePerson`, `storeClient`, `people`. `store()` aceita `people_id` opcional (vincula person ao tenant via `tenant_id`), `has_schema` (cria schema PostgreSQL apenas se true) e `plan_id` (nullable, validado com `exists:plans,id`); persiste e retorna `plan_id` no 201. `index()` usa model `Tenant::` em vez de `DB::table`; retorna `tenant_id`, `plan_id` e logo_urls via `TenantAvatarController::logoUrls`. `update()` aceita e retorna `plan_id` + logo_urls. `people(int $id)` lista pessoas filtradas por `tenant_id`, retorna avatarUrls via `PeopleAvatarController::avatarUrls()`. |
| `api/app/Http/Controllers/TenantAvatarController.php` | `store` (upload logo do gabinete → gera 3 versões: original.jpg, md.jpg, sm.jpg via Intervention Image), `destroy` (remove storage); `static logoUrls(?string)` retorna logo_original/md/sm |
...
| `api/app/Http/Controllers/CandidateController.php` | `index` (candidaturas por gabinete/master), `search`, `stats`, `cities`. `isMaster()` usa `$request->attributes->get('tenant')->slug` com fallback por Host. `search()`: busca por nome, cargo, ano, partido, UF e cidade (`unaccent(c.name)`); exclui `cy.role NOT ILIKE 'VICE-%'`; para não-master busca `people_candidacies` via query separada e usa `WHERE cy.id IN (...)`. `searchPersons()`: busca em `maps.candidates` por nome (não candidaturas). `candidaciesByPerson()`: lista candidaturas de um `maps.candidate` pelo `candidate_id`, retorna `party_color_gradient`. `stats()`: aceita `city_id`, `zone_id`, `voting_location_id`; escopo aplicado em prioridade: VL > zona > cidade > padrão cargo; `candidateScopeFilter` aplica o mesmo escopo na CTE `candidate_by_round`; resultado armazenado com `Cache::forever` por chave `candidacy_stats_{id}_city_{cid}_zone_{zid}_vl_{vlid}`; CTE `party_ids` + `scope_by_round` com `IN (SELECT id FROM party_ids)` em vez de JOIN; **fallback para candidato sem votos no escopo**: quando `$rows` vazio e o escopo tem dados de outros candidatos, retorna stats com `qty_votes=0` mas com totais reais de `total_valid`, `qty_blank`, `qty_null`, `qty_total` (evita tela vazia quando o candidato não teve votos naquela zona/colégio específico) |
...
| `api/app/Http/Controllers/AttendanceController.php` | CRUD de atendimentos + sub-recursos people/history — `index` (filtros: status, priority, people_id, date_from, date_to), `show`, `store` (auto-cria history), `update` (gera history se status mudar; auto resolved_at), `destroy`, `reorder`, `peopleIndex`/`peopleStore`/`peopleDestroy`, `historyIndex`/`historyStore`; `format()` inclui `priority`, `order`, `people[]` |
| `api/app/Http/Controllers/AttendanceAddressController.php` | CRUD de endereços polimórficos do atendimento (modulo=attendances) |
| `api/app/Http/Controllers/AttendanceContactController.php` | CRUD de contatos polimórficos do atendimento (modulo=attendance) |
| `api/app/Http/Controllers/AttendanceDocumentController.php` | CRUD de documentos polimórficos do atendimento (modulo=attendance) |
| `api/app/Http/Controllers/AttendanceNoteController.php` | CRUD de notas do atendimento (modulo=attendances) |
| `api/app/Http/Controllers/CityController.php` | `search` (`maps.cities`), `zones(cityId)` (zonas com qty_votes por candidacy + **campo `geometry` GeoJSON** em ambos os modos; filtra `v.year` para partition pruning; retorna collection mapeada com `json_decode($z->geometry)`), `votingLocations(cityId)` (colégios com qty_votes por candidacy + **`latitude`/`longitude`** em ambos os modos; `zone_id` opcional) |
| `api/app/Http/Controllers/AddressMapController.php` | `index(Request)`: retorna marcadores de endereço para o mapa; `?modulo=people` → join `gabinete_master.people`, `?modulo=attendances` → join `gabinete_master.attendances`; filtra `lat/lng NOT NULL`, `active=true`, `deleted_at IS NULL`; retorna id, record_id, lat, lng, logradouro, numero, bairro, cidade, name |
| `api/app/Http/Controllers/StateController.php` | geometry($uf) — retorna GeoJSON do estado |
| `api/app/Models/` | People, User, PersonalAccessToken, Permission, PermissionAction, PersonFile, Party, Candidate, Candidacy, PeopleCandidacy, SplitCandidacy, Attendance, AttendancePeople, AttendanceHistory, City, State, Zone, Section, VotingLocation, Vote, Gender |
| `api/app/Providers/AppServiceProvider.php` | `Sanctum::usePersonalAccessTokenModel(PersonalAccessToken::class)` |
| `api/database/migrations/` | migrations 2026_03_13_* numeradas 000001–000054 (gabinete) e 000101–000121 (maps) |
...
- URL: `https://servicodados.ibge.gov.br/api/v3/malhas/paises/BR?formato=application/vnd.geo+json&resolucao=5` (API IBGE, sem filtro necessário)
...
**UF → código IBGE estado** (mapeamento em `UF_TO_IBGE` — definido tanto em `mapa-do-voto-map.tsx` quanto em `brazil-map.tsx`):
