...
| `src/components/financeiro/fin-wallet-tab.tsx` | Aba da Carteira Financeira — filtro por Pessoa (autocomplete), totalizadores (Total Saídas / Total Entradas / Saldo), tabela de lançamentos (Data, Pessoa, Descrição, Origem, Entrada, Saída). Carrega `GET /fin-wallets`; botão "Novo Lançamento" abre `FinWalletModal`. |
| `src/components/financeiro/fin-wallet-modal.tsx` | Modal de criação de lançamento manual na carteira. Campos: Pessoa (autocomplete), Tipo (Entrada/Saída), Valor R$ (Modo Calculadora), Data (BirthDatePicker), Descrição, Banco, Conta Financeira, Modalidade. `POST /fin-wallets` — valida saldo na API ao salvar. |
...
Aba **Finanças** exibe `<FinMegaMenu />` acima do conteúdo. Seção ativa persiste em `localStorage` (`mapadovoto:finSection`). Seções disponíveis: painel principal (Títulos a Pagar), `fin-banks`, `fin-payment-methods`, `fin-payment-method-types`, `fin-departments`, `fin-accounts`, `fin-extract`, **`fin-wallet`**.
