...
<!-- Atualizado em: 31/03/2026 (logo do gabinete: upload/remoção via TenantAvatarController + logo_path em tenants; plan_id em tenants + aba Plano no gabinete-edit-modal; aniversários gerados dinamicamente na agenda; people-modal sem lógica de evento de aniversário; LandPlot para Planos) -->
...
| `src/components/people/people-modal.tsx` | Modal de pessoas: Create (small) + Detail (large 2 painéis). Abas: Geral, Contatos, Endereços, Documentos, Notas, Arquivos, Usuário, Permissões. Aba Endereços: layout 2 colunas com **BrasilAPI v2** (substituiu ViaCEP) + mapa Leaflet; CEP armazenado como dígitos puros; **geocodificação via Google Maps API** (`VITE_GOOGLE_MAPS_KEY`) com endereço completo (logradouro + número + bairro + cidade + UF + CEP); ao salvar novo endereço, geocodifica automaticamente; botão "Localizar no mapa" removido; `handleLocate` tenta BrasilAPI coordinates primeiro, cai em geocode se não houver. Aba Contatos: quando tipo selecionado for WhatsApp ou qualquer tipo com máscara, salva dígitos puros; exibe WhatsApp com `formatPhoneNumber` (`react-phone-number-input`). Aba Permissões: grid 3 colunas de Frame cards com Checkbox por ação. `PeopleModalProps` e `CreatePersonModal` aceitam `tenantId?: number`; ao criar pessoa com `tenantId` inclui `tenant_id` no payload do POST `/people`. **Sem lógica de evento de aniversário** — aniversários são gerados dinamicamente pela `agenda-tab` a partir de `birth_date`. |
...
| `src/components/attendances/attendance-tab.tsx` | Aba Atendimentos — carrega `GET /attendances`, filtragem client-side via `applyAttendanceFilters()`. Header com breadcrumb, badge de contagem, botões Pesquisar e Novo Registro. Renderiza `AttendanceDataGrid` + `AttendanceModal` + `AttendanceFilterModal`. `handleSaved`: ao criar, reabre modal para edição imediata. `handleReorder`: DnD otimístico com `PUT /attendances/reorder`. |
...
| `src/components/attendances/attendance-modal.tsx` | Modal criar/editar atendimento com abas (Geral, Pessoas, Histórico, Contatos, Documentos, Endereços, Notas). |
