...
<!-- Atualizado em: 03/04/2026 04:28 -->
...
| `src/components/map/brazil-map.tsx` | Mapa Leaflet standalone usado na aba **Mapa novo**. Props: `candidate: Candidate \| null`, `onCityClick?: (name: string) => void`. Handle ref (`BrazilMapHandle`): `zoomIn`, `zoomOut`, `fitBrazil`, **`fitState`** (volta zoom ao estado do candidato), **`focusCity(name)`** (destaca e centraliza município pelo nome). `MapCore`: sem candidato → carrega contorno do Brasil via API IBGE (`servicodados.ibge.gov.br/api/v3/malhas/paises/BR`); com candidato municipal → polígono da cidade via tbrugz; com candidato estadual → divisões de municípios (tbrugz) + contorno do estado (via `GET /api/states/{uf}/geometry`), com clique nos municípios (destaque + `onCityClick`). `stateBoundsRef` guarda bounds do estado para `fitState`. `focusCityRef` expõe função de foco por nome normalizado (sem acento). `UF_TO_IBGE` + `normalizeName` para match de cidades. |
...
| GET | `/api/candidacies/{id}/stats?city_id=&zone_id=&voting_location_id=` | Bearer | — | Retorna votos por turno: qty_votes, %, brancos, nulos, legenda, total partido, status TSE — query única CTE com partition pruning por year; escopo: `voting_location_id` > `zone_id` > `city_id` > padrão cargo; resultado cacheado com `Cache::forever` por chave `candidacy_stats_{id}_city_{cid}_zone_{zid}_vl_{vlid}` |
...
| GET | `/api/cities/{id}/zones?candidacy_id=` | Bearer | — | Lista zonas eleitorais de uma cidade; com `candidacy_id` retorna `qty_votes` agregado por zona (maior turno, filtrando por year para partition pruning); sem candidacy retorna apenas id + zone_number |
| GET | `/api/cities/{id}/voting-locations?candidacy_id=&zone_id=` | Bearer | — | Lista colégios eleitorais de uma cidade (join voting_locations → zones → city); com `candidacy_id` retorna `qty_votes` agregado; `zone_id` opcional filtra por zona; ordenado por qty_votes DESC, nome ASC |
...
| `api/app/Http/Controllers/CityController.php` | `search` (`maps.cities`), `zones(cityId)` (zonas com qty_votes por candidacy, filtra `v.year` para partition pruning), `votingLocations(cityId)` (colégios com qty_votes por candidacy; `zone_id` opcional) |
