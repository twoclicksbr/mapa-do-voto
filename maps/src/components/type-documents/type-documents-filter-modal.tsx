...
export interface TypeDocumentsFilters {
  filterId?:   string;
  filterName?: string;
  status?:     string[];
  validity?:   string;
...
interface TypeDocumentsFilterModalProps {
  open: boolean;
  filters: TypeDocumentsFilters;
  onClose: () => void;
  onApply: (filters: TypeDocumentsFilters) => void;
}

export function TypeDocumentsFilterModal({ open, filters, onClose, onApply }: TypeDocumentsFilterModalProps) {
  ...
  useEffect(() => {
    ...
    setValidity(filters.validity ?? "all");
  ...
  const hasFilters = !!(filterId || filterName || statuses.length || validity !== "all" || dateValue);
      ...
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Pesquisa Avançada — Tipos de Documento
          </DialogTitle>
        </DialogHeader>
        <DialogBody className="space-y-4">

          {/* Linha 1: ID(2) + Nome(7) + Validade(1) + Status(2) */}
