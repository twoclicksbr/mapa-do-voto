...
interface TypeAddressesDataGridProps {
  typeAddresses: TypeAddress[];
  isLoading: boolean;
  onSelectionChange?: (count: number) => void;
  onEdit?: (ta: TypeAddress) => void;
  onDelete?: (id: number) => void;
  onReorder?: (id: number, newOrder: number) => void;
}

export function TypeAddressesDataGrid({
  typeAddresses,
  isLoading,
  onSelectionChange,
  onEdit,
  onDelete,
  onReorder,
}: TypeAddressesDataGridProps) {
  const [data, setData] = useState<TypeAddress[]>(typeAddresses);
  const [pagination, setPagination] = useState<PaginationState>({ pageIndex: 0, pageSize: 10 });
  const [sorting, setSorting] = useState<SortingState>([]);
  const [columnOrder, setColumnOrder] = useState<string[]>(["drag", "select", "id", "name", "active", "actions"]);
  const [rowSelection, setRowSelection] = useState({});

  useEffect(() => { setData(typeAddresses); }, [typeAddresses]);
  ...
  const columns = useMemo<ColumnDef<TypeAddress>[]>(
    () => [
      {
        id: "drag",
        header: () => null,
  ...
  const table = useReactTable({
    ...
    getRowId: (row: TypeAddress) => String(row.id),
