...
interface PlansFilterModalProps {
  open: boolean;
  filters: PlansFilters;
  onClose: () => void;
  onApply: (filters: PlansFilters) => void;
}

export function PlansFilterModal({ open, filters, onClose, onApply }: PlansFilterModalProps) {
  ...
  const [hasSchema,     setHasSchema]     = useState<string>("all");
  ...
  useEffect(() => {
    ...
    setHasSchema(filters.hasSchema === true ? "yes" : filters.hasSchema === false ? "no" : "all");
  ...
  const hasFilters = !!(filterId || filterName || statuses.length || hasSchema !== "all" || dateValue);
  ...
  const handleApply = () => {
    onApply({
      ...
      hasSchema:  hasSchema === "yes" ? true : hasSchema === "no" ? false : null,
      dateField,
      dateValue,
    });
    onClose();
      ...
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Pesquisa Avançada — Planos
          </DialogTitle>
        </DialogHeader>
        <DialogBody className="space-y-4">

          {/* Linha 1: ID(2) + Nome(6) + CRM(2) + Status(2) */}
          <div className="grid grid-cols-12 gap-4">
            ...
            <div className="col-span-2 space-y-1.5">
              <Label>CRM</Label>
              <Select value={hasSchema} onValueChange={setHasSchema}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
