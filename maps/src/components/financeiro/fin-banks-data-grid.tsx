...
interface FinBanksDataGridProps {
  ...
  onEdit?: (bank: FinBank) => void;
  onDelete?: (id: number) => void;
  onReorder?: (id: number, newOrder: number) => void;
}

export function FinBanksDataGrid({
  banks,
  isLoading,
  onSelectionChange,
  onEdit,
  ...
  const columns = useMemo<ColumnDef<FinBank>[]>(
    () => [
      {
        id: "drag",
        header: () => null,
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-muted-foreground text-sm">
            {row.original.bank ?? <span className="italic text-xs">—</span>}
          </span>
        ),
      ...
      },
      {
        accessorKey: "agency",
        id: "agency",
        header: ({ column }) => <DataGridColumnHeader title="Agência" column={column} />,
        cell: ({ row }) => (
          <span className="font-mono text-sm text-muted-foreground">
            {row.original.agency ?? <span className="italic text-xs">—</span>}
          </span>
        ),
        meta: { skeleton: <Skeleton className="h-5 w-20" />, headerClassName: "w-[12%]", cellClassName: "w-[12%]" },
        enableSorting: false,
        enableHiding: false,
      },
      {
      ...
      },
      {
        ...
        meta: { skeleton: <Skeleton className="h-6 w-16" />, headerClassName: "w-[9%]", cellClassName: "w-[9%]" },
        enableSorting: true,
        enableHiding: false,
      },
      {
  ...
  const table = useReactTable({
    ...
    getRowId: (row: FinBank) => String(row.id),
