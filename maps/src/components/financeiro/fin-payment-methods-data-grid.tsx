...
export interface FinPaymentMethod {
  id: number;
  name: string;
  fin_bank_id: number | null;
  bank_name: string | null;
  fin_payment_method_type_id: number | null;
  type_name: string | null;
  order: number;
  active: boolean;
}

interface FinPaymentMethodsDataGridProps {
  methods: FinPaymentMethod[];
  isLoading: boolean;
  onSelectionChange?: (count: number) => void;
  onEdit?: (method: FinPaymentMethod) => void;
  onDelete?: (id: number) => void;
  onReorder?: (id: number, newOrder: number) => void;
}

export function FinPaymentMethodsDataGrid({
  methods,
  isLoading,
  onSelectionChange,
  onEdit,
  onDelete,
  onReorder,
}: FinPaymentMethodsDataGridProps) {
  const [data, setData] = useState<FinPaymentMethod[]>(methods);
  const [pagination, setPagination] = useState<PaginationState>({ pageIndex: 0, pageSize: 10 });
  const [sorting, setSorting] = useState<SortingState>([]);
  const [columnOrder] = useState<string[]>(["drag", "select", "id", "name", "type_name", "bank_name", "active", "actions"]);
  const [rowSelection, setRowSelection] = useState({});

  useEffect(() => { setData(methods); }, [methods]);
  ...
  const columns = useMemo<ColumnDef<FinPaymentMethod>[]>(
    () => [
      {
        id: "drag",
        header: () => null,
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-muted-foreground text-sm">
            {row.original.type_name ?? <span className="italic text-xs">—</span>}
          </span>
        ),
        meta: { skeleton: <Skeleton className="h-5 w-24" />, headerClassName: "w-[20%]", cellClassName: "w-[20%]" },
        enableSorting: true,
        enableHiding: false,
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-muted-foreground text-sm">
            {row.original.bank_name ?? <span className="italic text-xs">—</span>}
          </span>
        ),
        meta: { skeleton: <Skeleton className="h-5 w-24" />, headerClassName: "w-[20%]", cellClassName: "w-[20%]" },
        enableSorting: true,
  ...
  const table = useReactTable({
    ...
    getRowId: (row: FinPaymentMethod) => String(row.id),
