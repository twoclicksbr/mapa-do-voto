...
export function DepartmentsDataGrid({
  ...
  useEffect(() => { setData(departments); }, [departments]);
  ...
  const columns = useMemo<ColumnDef<Department>[]>(
    () => [
      ...
      },
      {
        ...
        meta: { skeleton: <Skeleton className="h-6 w-16" />, headerClassName: "w-[12%]", cellClassName: "w-[12%]" },
        enableSorting: true,
        enableHiding: false,
      },
      {
  ...
  const table = useReactTable({
    ...
    getRowId: (row: Department) => String(row.id),
