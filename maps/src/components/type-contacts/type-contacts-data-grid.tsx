...
interface TypeContactsDataGridProps {
  ...
  onEdit?: (tc: TypeContact) => void;
  onDelete?: (id: number) => void;
  onReorder?: (id: number, newOrder: number) => void;
}

export function TypeContactsDataGrid({
  typeContacts,
  isLoading,
  onSelectionChange,
  onEdit,
  onDelete,
  onReorder,
}: TypeContactsDataGridProps) {
  const [data, setData] = useState<TypeContact[]>(typeContacts);
  const [pagination, setPagination] = useState<PaginationState>({ pageIndex: 0, pageSize: 10 });
  const [sorting, setSorting] = useState<SortingState>([]);
  const [columnOrder, setColumnOrder] = useState<string[]>(["drag", "select", "id", "name", "mask", "active", "actions"]);
  const [rowSelection, setRowSelection] = useState({});

  useEffect(() => { setData(typeContacts); }, [typeContacts]);
  ...
  const columns = useMemo<ColumnDef<TypeContact>[]>(
    () => [
      {
        id: "drag",
        header: () => null,
  ...
  const table = useReactTable({
    ...
    getRowId: (row: TypeContact) => String(row.id),
