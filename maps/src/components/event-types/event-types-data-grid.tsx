...
export function EventTypesDataGrid({
  eventTypes,
  isLoading,
  onSelectionChange,
  onEdit,
  onDelete,
  onReorder,
}: EventTypesDataGridProps) {
  const [data, setData] = useState<EventType[]>(eventTypes);
  const [pagination, setPagination] = useState<PaginationState>({ pageIndex: 0, pageSize: 10 });
  const [sorting, setSorting] = useState<SortingState>([]);
  const [columnOrder, setColumnOrder] = useState<string[]>(["drag", "select", "id", "color", "name", "all_day", "active", "actions"]);
  const [rowSelection, setRowSelection] = useState({});

  useEffect(() => { setData(eventTypes); }, [eventTypes]);
  ...
  const columns = useMemo<ColumnDef<EventType>[]>(
    () => [
      ...
      },
      {
        accessorKey: "color",
        id: "color",
        header: () => <span className="text-muted-foreground text-sm">Cor</span>,
        cell: ({ row }) => (
          <span
        ...
        meta: { skeleton: <Skeleton className="h-5 w-5 rounded-full" />, headerClassName: "w-[6%]", cellClassName: "w-[6%]" },
        enableSorting: false,
        enableHiding: false,
      },
      {
      ...
      },
      {
        accessorKey: "all_day",
        id: "all_day",
        header: ({ column }) => <DataGridColumnHeader title="Dia Inteiro" column={column} />,
        cell: ({ row }) =>
          row.original.all_day ? (
            <Badge variant="warning" appearance="light">Sim</Badge>
          ) : (
            <span className="text-muted-foreground text-sm">—</span>
          ),
        meta: { skeleton: <Skeleton className="h-6 w-10" />, headerClassName: "w-[10%]", cellClassName: "w-[10%]" },
        enableSorting: false,
        enableHiding: false,
      },
      {
  ...
  const table = useReactTable({
    ...
    getRowId: (row: EventType) => String(row.id),
