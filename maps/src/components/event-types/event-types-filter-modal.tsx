...
export interface EventTypesFilters {
  filterId?:   string;
  filterName?: string;
  status?:     string[];
  color?:      string;
...
interface EventTypesFilterModalProps {
  open: boolean;
  filters: EventTypesFilters;
  colors: ColorOption[];
  onClose: () => void;
  onApply: (filters: EventTypesFilters) => void;
}

export function EventTypesFilterModal({ open, filters, colors, onClose, onApply }: EventTypesFilterModalProps) {
  ...
  const hasFilters = !!(filterId || filterName || statuses.length || color !== "all" || dateValue);
      ...
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Pesquisa Avançada — Tipos de Evento
          </DialogTitle>
        </DialogHeader>
        <DialogBody className="space-y-4">

          {/* Linha 1: ID(2) + Nome(8) + Status(2) */}
          ...
          {/* Linha 2: Pesquisar por(2) + Período(4) + Cor(3) */}
          <div className="grid grid-cols-12 gap-4">
            <div className="col-span-2 space-y-1.5">
                  ...
                  <SelectValue>
                    {color === "all" ? (
                      ...
                      <span className="flex items-center gap-2">
                        ...
                        <span>{colors.find(c => c.color === color)?.name ?? color}</span>
                      </span>
                    )}
                  </SelectValue>
                </SelectTrigger>
